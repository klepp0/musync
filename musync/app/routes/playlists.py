...
def create_playlist(
    payload: PlaylistCreateRequest, origin: Literal["spotify", "tidal"]
) -> Playlist:
    match origin:
        case "spotify":
            session = SpotifySession()
    ...
    new_playlist = session.create_playlist(payload.title)
...
def add_tracks_to_playlist(tracks: List[Track], origin: Literal["spotify", "tidal"]):
    match origin:
        case "spotify":
            session = SpotifySession()
        case "tidal":
